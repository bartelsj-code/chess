...
class Engine(Player):
    ...
    def doTurn(self, lastmove):
        ...
        for haltDepth in range(1, self.depthCap):
            ...
            self.minimax(gsHolder, alpha, beta, 0, haltDepth)
            if self.quitting:
                break
        ...
        if self.isMaximizing(gameStateHolder):
            ...
            for child in gameStateHolder.evaluatedChildren:
                ...
                if eval > maxEval:
                    maxEval = eval
                    alpha = eval
                if self.shouldPruneRemainingMoves(maxEval, beta):
                    pruned = True
                    break
            ...
            for child in gameStateHolder.evaluatedChildren:
                ...
                if eval < minEval:
                    minEval = eval
                    beta = minEval
                if self.shouldPruneRemainingMoves(alpha, minEval):
                    pruned = True
                    break
    ...
    def shouldPruneRemainingMoves(self, growing, shrinking):
        # return False
